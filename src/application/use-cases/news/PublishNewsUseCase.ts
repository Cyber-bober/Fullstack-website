...
export class PublishNewsUseCase {
  ...
  async createPost(input: NewsInput, authorId: string): Promise<NewsDTO> {
    ...
    if (!author.canManageNews()) throw new Error('Only editors and admins can create posts');
    if (!input.title || input.title.length < 3) throw new Error('Title must be at least 3 characters');
    if (!input.content || input.content.length < 10) throw new Error('Content must be at least 10 characters');
    const category = (input.category as NewsCategory) || NewsCategory.GENERAL;
    const post = new NewsPost(crypto.randomUUID(), authorId, input.title, input.content, new Date(), new Date(), category, input.imageUrl, input.matchId);
    post.publish();
    const created = await this.newsRepo.create(post);
    return toNewsDTO(created, { id: author.id, username: author.username, fullName: author.fullName });
  }
  ...
  async updatePost(postId: string, input: Partial<NewsInput>, actorId: string): Promise<NewsDTO> {
    ...
    // EDITOR can edit all posts, ADMIN too
    if (actor.role !== 'ADMIN' && actor.role !== 'EDITOR' && post.authorId !== actorId) {
    ...
    return toNewsDTO(updated, { id: actor.id, username: actor.username, fullName: actor.fullName });
  }
  ...
  async togglePublish(postId: string, actorId: string): Promise<NewsDTO> {
    ...
    const post = await this.newsRepo.findById(postId);
    if (!post) throw new Error('Post not found');
    if (post.isPublished) post.unpublish(); else post.publish();
    const updated = await this.newsRepo.update(post);
    return toNewsDTO(updated, { id: actor.id, username: actor.username, fullName: actor.fullName });
  }

  async getFeed(page: number, pageSize: number): Promise<PaginatedNews> { return this.newsRepo.findPublished(page, pageSize); }
