        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="form-group">
            ...
            <input type="text" value={formData.username} onChange={(e) => {
              setFormData({ ...formData, username: e.target.value }); setUsernameError("");
            ...
            {usernameError ? (
              <p style={{ color: "#ef4444", fontSize: "13px", marginTop: "4px" }}>⚠️ {usernameError}</p>
            ) : (<small className="text-gray">Только латинские буквы, цифры и _</small>)}
          </div>

          <div className="form-group"><label>Полное имя</label><input type="text" value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} required /></div>
          ...
          <div className="form-group"><label>Позиция</label><input type="text" value={formData.position} onChange={(e) => setFormData({ ...formData, position: e.target.value })} /></div>
